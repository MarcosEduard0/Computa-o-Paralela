...
class LE:
    ...
    def leitorBloqueado(self, id):
        ...
        if(self.escritores == 0):
            print("ERRO: leitor " + str(id) +
                  " bloqueado quando nao ha escritores!")

    def leitorEscritorBloqueado(self, id):
        '''Recebe o id do leitorEscritor. Verifica se a decisao de bloqueio esta correta.'''
        if(self.escritores == 0 and self.leitores == 0):
            print("ERRO: leitorEscritor " + str(id) +
                  " bloqueado quando nao ha escritores!")

    def escritorBloqueado(self, id):
        '''Recebe o id do escritor. Verifica se a decisao de bloqueio esta correta.'''
        if(self.escritores == 0) and (self.leitores == 0):
            print("ERRO: escritor " + str(id) +
                  " bloqueado quando nao ha escritores nem leitores!")

    def leitorLendo(self, id):
        '''Recebe o id do leitor, verifica se pode ler e registra que esta lendo.'''
        if(self.escritores > 0):
            print("ERRO: leitor " + str(id) +
                  " esta lendo quando ha escritor escrevendo!")
        self.leitores += 1
    ...
    def escritorEscrevendo(self, id):
        ...
        if(self.escritores > 0 or self.leitores > 0):
            print("ERRO: escritor " + str(id) +
                  " esta escrevendo quando ha outro escritor ou leitores!")
        self.escritores += 1

    def leitorEscritorEscrevendo(self, id):
        '''Recebe o id do escritor, verifica se pode escrever e registra que esta escrevendo'''
        if(self.escritores > 0 or self.leitores > 0):
            print("ERRO: leitoEscritor " + str(id) +
                  " esta escrevendo quando ha outro escritor ou leitores!")
        self.escritores += 1
    ...
    def leitorEscritorSaindo(self, id):
        '''Recebe o id do LeitorEscritor e registra que terminou a leitura e escrita.'''
        self.escritores -= 1
